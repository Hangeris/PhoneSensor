...
namespace BarGraph.VittorCloud
{
    ...
    public class BarGraphGenerator : MonoBehaviour
    {
        ...
        List<BarGraphDataSet> ListOfDataSet;
        ...
        {
            if (graphGO != null)
                graphGO.GraphUpdated -= OnGraphAnimationCompleted;
        }
        #endregion
        ...
        void Update()
        {
            if (graphGO != null)
            {
                graphGO.transform.position = TheOffset;
            }
        }
        ...
        void ParseTheDataset()
        {
            ...
            ValidateRange();
            graphGO.FetchYPointValues(yMinValue, yMaxValue);
        }
        ...
        {
            graphGO = Instantiate(GraphRef, transform.position, Quaternion.identity);
            graphGO.transform.rotation = this.transform.rotation;
            graphGO.transform.parent = this.transform;
            ...
            graphGO.setBarScale(barScaleFactor);
            graphGO.InitGraphBox(XLength, YLength, ZLength, segmentSizeOnXaxis, segmentSizeOnYaxis, segmentSizeOnZaxis);
            graphGO.SetBarRef(barPrefab);
            graphGO.InitXAxis(xMaxSize, xStart, segmentSizeOnXaxis, offsetBetweenXRow);
            graphGO.InitYAxis(yMaxSize, yStart, segmentSizeOnYaxis);
            graphGO.InitZAxis(zMaxSize, zStart, segmentSizeOnZaxis, offsetBetweenZRow, xStart, segmentSizeOnXaxis);
        ...
        public void AssignGraphAxisName()
        {
            for (int i = 0; i < ListOfDataSet.Count; i++)
            {
                for (int j = 0; j < ListOfDataSet[i].ListOfBars.Count; j++)
                {
                    graphGO.AssignAxisName(j, i, ListOfDataSet[i].ListOfBars[j].XValue, ListOfDataSet[i].GroupName);
                }
            }
        }
            ...
            for (int i = 0; i < ListOfDataSet.Count; i++)
            {
                for (int j = 0; j < ListOfDataSet[i].ListOfBars.Count; j++)
                {
                    ...
                    if (barColorProperty == BarColor.SolidColor)
                        yield return StartCoroutine(graphGO.GenerateGraphBarWithAnimTypeOne(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barColor));
                    ...
                    else if (barColorProperty == BarColor.HeightWiseGradient)
                    {
                        ...
                        if (graphAnimation == AnimatioType.animationWithGradient)
                        {
                            yield return StartCoroutine(graphGO.GenerateGraphBarWithAnimTypeThree(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, barcolor, gredientStartColor));
                        }
                        else
                            yield return StartCoroutine(graphGO.GenerateGraphBarWithAnimTypeOne(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, barcolor));
                    }
                    else
                        yield return StartCoroutine(graphGO.GenerateGraphBarWithAnimTypeOne(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barMaterial));
                    yield return new WaitForSeconds(0.2f);
                }
            }
            graphGO.GraphUpdated();
            yield return null;
        }
        ...
        public void CreateBarsWithAnimTypeTwo()
        {
            for (int i = 0; i < ListOfDataSet.Count; i++)
            {
                for (int j = 0; j < ListOfDataSet[i].ListOfBars.Count; j++)
                {
                    float yscaleFactor = (((yMaxSize - 1) * segmentSizeOnYaxis) + yStart) / (yMaxValue - yMinValue);
                    if (barColorProperty == BarColor.SolidColor)
                        graphGO.GenerateBarWithAnimTypeTwo(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barColor);
                    else if (barColorProperty == BarColor.HeightWiseGradient)
                    {
                        float time = (ListOfDataSet[i].ListOfBars[j].YValue - yMinValue) / (yMaxValue - yMinValue);
                        Color barcolor = HeightWiseGradient.Evaluate(time);
                        graphGO.GenerateBarWithAnimTypeTwo(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, barcolor);
                    }

                    else
                        graphGO.GenerateBarWithAnimTypeTwo(j, i, ListOfDataSet[i].ListOfBars[j].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barMaterial);

                }
            }

            StartCoroutine(graphGO.AnimateBarsWithAnimTypeTwo(animationSpeed));
        ...
        public void AddNewDataSet(int dataSetIndex, int xyValueIndex, float yValue)
        {
            if (modifyData)
            {
                ...
                if (barColorProperty == BarColor.HeightWiseGradient)
                {
                    ...
                    if (graphAnimation == AnimatioType.animationWithGradient)
                    {
                        graphGO.UpdateBarHeight(xyValueIndex, dataSetIndex, ListOfDataSet[dataSetIndex].ListOfBars[xyValueIndex].YValue, yscaleFactor, animationSpeed, yMinValue, barcolor, gredientStartColor);
                    }
                    else
                    {
                        graphGO.UpdateBarHeight(xyValueIndex, dataSetIndex, ListOfDataSet[dataSetIndex].ListOfBars[xyValueIndex].YValue, yscaleFactor, animationSpeed, yMinValue, barcolor);
                    }
                }
                else
                {
                    graphGO.UpdateBarHeight(xyValueIndex, dataSetIndex, ListOfDataSet[dataSetIndex].ListOfBars[xyValueIndex].YValue, yscaleFactor, animationSpeed, yMinValue);
                }
            }
        }
        ...
        public void ModifyGraph()
        {
            for (int i = 0; i < ListOfDataSet.Count; i++)
            {
                ...
                graphGO.RemoveAndShiftXpoints(ListOfDataSet[i].ListOfBars[lastIndex].XValue);


                float yscaleFactor = (yMaxSize * segmentSizeOnYaxis) / (yMaxValue - yMinValue);
                if (barColorProperty == BarColor.SolidColor)
                    graphGO.GenerateBarWithAnimTypeTwo(lastIndex, i, ListOfDataSet[i].ListOfBars[lastIndex].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barColor);
                else
                    graphGO.GenerateBarWithAnimTypeTwo(lastIndex, i, ListOfDataSet[i].ListOfBars[lastIndex].YValue, yscaleFactor, animationSpeed, yMinValue, xMaxSize, ListOfDataSet[i].barMaterial);
